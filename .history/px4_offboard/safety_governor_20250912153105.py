...
class SafetyGovernor(Node):
    ...
    def __init__(self) -> None:
        ...
        # [x, y, z] with z down
        self.people_ned: List[np.ndarray] = [  # static people in local NED [x, y, z] (z down)
        ...
        self.wp_a = np.array([0.0, 0.0, -5.0], dtype=float)   # line start (x,y,z NED)
        self.wp_b = np.array([20.0, 0.0, -5.0], dtype=float)  # line end (x,y,z NED)
        self.current_target = self.wp_b.copy()                 # which end we’re heading to now
        self.wp_switch_threshold = 0.5  # m; swap ends when within this distance

        # --- Simple P gains for position -> velocity ---
    self.kp_xy = 1.0  # P gain: pos error -> horizontal vel (raise to increase cruise speed)
        self.kp_z = 0.5   # P gain: pos error -> vertical vel (z down)

        # --- Minimal avoidance settings ---
        self.safety_radius = 1.5   # m; forbidden near-person radius for the base line
        self.circle_radius = 1.75  # m; fixed circle to skim around a person
        self.circle_active = False # in avoidance-circle mode
        self.circle_center = np.zeros(2, dtype=float)  # [x,y] center of active circle
        self.circle_dir = 1  # +1 CCW, -1 CW along circle
        self.circle_lookahead = 0.5  # m along the circle used as a smooth target
    self.base_lookahead = 3.0    # m along the base line to help recentre and set cruise speed
        ...
        self.kv_xy = 0.6        # scales horizontal speed limit with horizontal distance to people
        self.kv_z = 0.6         # scales vertical speed limit with vertical distance to people
        self.v_cap_xy = 3.0     # hard cap on horizontal speed [m/s]
        self.v_cap_z = 1.5      # hard cap on vertical speed [m/s] (z down)

        # --- Acceleration limits as slew-rate on velocity ---
        self.ka_xy = 0.8         # scales horizontal accel limit with horizontal distance to people
        self.ka_z = 0.8          # scales vertical accel limit with vertical distance to people
        self.a_cap_xy = 2.0       # hard cap on horizontal accel [m/s^2]
        self.a_cap_z = 1.0        # hard cap on vertical accel [m/s^2]

        # --- State ---
        self.vehicle_position = None  # last local position from PX4 (NED); type: VehicleLocalPosition | None
        self.prev_v_cmd = np.zeros(3, dtype=float)  # previous commanded velocity (for slewrate)
        ...
        # --- Control timer (20 Hz) ---
        self.dt = 0.05  # control period [s] (20 Hz)
    ...
    def cmdloop_callback(self) -> None:
        ...
        p = np.array([self.vehicle_position.x, self.vehicle_position.y, self.vehicle_position.z], dtype=float)  # NED
        ...
        p_xy = np.array([p[0], p[1]], dtype=float)                    # current xy position
        t_xy = np.array([self.current_target[0], self.current_target[1]], dtype=float)  # target end xy
        a_xy = np.array([self.wp_a[0], self.wp_a[1]], dtype=float)    # base line start xy
        b_xy = np.array([self.wp_b[0], self.wp_b[1]], dtype=float)    # base line end xy
        d_line = b_xy - a_xy                                          # base line vector
        L = float(np.linalg.norm(d_line))                             # base line length
        if L > 1e-6:
            u_line = d_line / L                                       # unit direction along base line
            s_on = float(np.dot(p_xy - a_xy, u_line))                 # projection (arc-length) of p onto line
            s_on = max(0.0, min(L, s_on))                             # clamp to segment
            dir_sign = 1.0 if np.allclose(self.current_target, self.wp_b) else -1.0  # + forward, - backward
            s_goal = max(0.0, min(L, s_on + dir_sign * self.base_lookahead))         # lookahead along line
            line_xy = a_xy + u_line * s_goal                          # xy target on base line
        ...
        if not self.circle_active:
            ...
            hit_center = None                                         # circle center that blocks the path
            best_s = float('inf')                                     # nearest intersection along the path
            seg = t_xy - p_xy                                         # straight path vector
            seg_len = float(np.linalg.norm(seg))                      # path length
            if seg_len > 1e-6:
                u = seg / seg_len
                for c in self.people_ned:
                    c_xy = np.array([c[0], c[1]], dtype=float)
                    # skip if behind start or beyond target
                    w = c_xy - p_xy                                  # vector from start to person
                    s = float(np.dot(w, u))                           # along-track distance to closest point
                    if s < 0.0 or s > seg_len:
                        continue
                    # check distance from segment to center
                    closest = p_xy + u * s                            # closest point on path to person
            ...
            if hit_center is not None:
                ...
                t1, t2 = self._tangent_points_to_circle(p_xy, hit_center, self.circle_radius)  # entry tangents
                # choose tangent that better heads toward the target
                to_t = (t_xy - p_xy)                                  # direction toward final target
                if np.linalg.norm(to_t) < 1e-6:
                    pick = t1
                else:
                    v1 = t1 - p_xy                                    # from current pos to tangent 1
                    v2 = t2 - p_xy                                    # from current pos to tangent 2
                ...
                r = pick - hit_center                                 # radial vector from center to tangent
                t_ccw = np.array([-r[1], r[0]], dtype=float)          # CCW tangent at pick
                v = pick - p_xy                                       # chosen entry direction
                self.circle_dir = 1 if float(np.dot(v, t_ccw)) >= 0.0 else -1  # decide CW/CCW
                self.circle_center = hit_center
                self.circle_active = True
        else:
            # follow a point slightly ahead along the circle for smooth motion
            r = p_xy - self.circle_center                             # current radial vector
            R = max(1e-3, self.circle_radius)                         # circle radius (avoid div0)
            if float(np.linalg.norm(r)) < 1e-6:
                r = np.array([R, 0.0])
            ang = math.atan2(r[1], r[0])                               # current polar angle on circle
            d_ang = self.circle_lookahead / R * float(self.circle_dir) # angle step for lookahead
            ang_ahead = ang + d_ang                                    # ahead angle
            ahead = self.circle_center + R * np.array([math.cos(ang_ahead), math.sin(ang_ahead)])  # ahead point
            ...
            seg = t_xy - p_xy                                         # direct path to target
            seg_len = float(np.linalg.norm(seg))
            if seg_len > 1e-6:
                u = seg / seg_len                                     # unit vector along path
                s_clear = max(0.5, float(self.circle_lookahead))       # m to skip before checking clearance
                s_clear = min(s_clear, seg_len * 0.9)                  # cap within segment
                p_ahead = p_xy + u * s_clear                           # shifted start for clearance check
            else:
                p_ahead = p_xy
        ...
        err = nav_target - p  # position error (target - current)
        v_cmd = np.array([
            self.kp_xy * err[0],
            self.kp_xy * err[1],
            self.kp_z * err[2],
        ...
        v_cmd = self._limit_velocity(v_cmd, v_max_xy, v_max_z)  # clamp speeds

        # Acceleration (slew-rate) limiting
        a_max_xy = min(self.a_cap_xy, self.ka_xy * d_h_min)
        a_max_z = min(self.a_cap_z, self.ka_z * d_v_min)
        v_cmd = self._limit_accel(self.prev_v_cmd, v_cmd, a_max_xy, a_max_z, self.dt)  # slew-rate limit
