...
class SafetyGovernorNoAvoidance(Node):
    ...
    def _limit_velocity(self, v: np.ndarray, v_max_xy: float, v_max_z: float) -> np.ndarray:
        v_lim = v.copy()
        # 1) Global 3D speed cap: limit the norm of [vx, vy, vz] using v_max_xy as the overall speed limit
        #    (so vertical motion reduces available horizontal speed to keep total speed bounded).
        speed = float(np.linalg.norm(v_lim))
        if math.isfinite(v_max_xy) and v_max_xy >= 0.0 and speed > v_max_xy and speed > 1e-4:
            v_lim *= (v_max_xy / speed)

        # 2) Vertical cap: additionally constrain vz independently.
        if math.isfinite(v_max_z) and v_max_z >= 0.0:
            v_lim[2] = float(np.clip(v_lim[2], -v_max_z, v_max_z))
        return v_lim
