...
class SafetyGovernorNoAvoidance(Node):
    ...
    def cmdloop_callback(self) -> None:
        ...
        # 3D distance to nearest person
        if self.people_ned:
            d_min = min(float(np.linalg.norm(p - q)) for q in self.people_ned)
        else:
            d_min = float('inf')

        # 3D speed limit
        v_max = min(self.v_cap_xy, self.kv_xy * d_min)
        v_cmd = self._limit_velocity(v_cmd, v_max)

        # 3D acceleration (slew-rate) limit
        a_max = min(self.a_cap_xy, self.ka_xy * d_min)
        v_cmd = self._limit_accel(self.prev_v_cmd, v_cmd, a_max, self.dt)
