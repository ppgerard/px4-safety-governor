...
class SafetyGovernorNoAvoidance(Node):
    ...
    - 8 m/s max, 3.0 m/s à moins de 4 m d'un humain.
    """
    ...
    def __init__(self) -> None:
        ...
        self.v_cap_near = 3.0      # m/s when inside safety_radius
    ...
    def cmdloop_callback(self) -> None:
        ...
        # 3D distance to nearest person (sphere in NED)
        if self.people_ned:
            d_min = min(float(np.linalg.norm(p - q)) for q in self.people_ned)
        else:
            d_min = float('inf')

        # Choose cap based on 3D sphere membership
