...
class SafetyGovernor(Node):
    ...
    def __init__(self) -> None:
        ...
        self.avoid_radius = 3.0
        self.k_avoid = 1.0  # strength of repulsive velocity (m/s per meter of penetration)
    ...
    self.armed_set = False
    self.bootstrap_cycles = 0  # ensure a little stream before mode/arm
    ...
    self.pub_vehicle_cmd = self.create_publisher(VehicleCommand, 'fmu/in/vehicle_command', self.qos_pub)

        # --- Subscribers ---
        self.sub_pos = self.create_subscription(
            VehicleLocalPosition, 'fmu/out/vehicle_local_position', self.cb_vehicle_pos, self.qos_sub
        )
        self.sub_status = self.create_subscription(
            VehicleStatus, 'fmu/out/vehicle_status', self.cb_vehicle_status, self.qos_sub
        )
    ...
    def control_loop(self) -> None:
        ...
        # Try to set OFFBOARD and ARM automatically after a short bootstrap
        self.bootstrap_cycles += 1
        if self.bootstrap_cycles == int(1.0 / self.dt):
            # after ~1s of streaming
            if not self.offboard_set:
                self.set_offboard_mode()
            if not self.armed_set:
                self.arm()

        # Status debug (throttled)
        if self.vehicle_status is not None and (self.bootstrap_cycles % int(2.0 / self.dt) == 0):
            self.get_logger().info(
                f"nav_state={self.vehicle_status.nav_state} (OFFBOARD={VehicleStatus.NAVIGATION_STATE_OFFBOARD}), "
                f"arming={self.vehicle_status.arming_state} (ARMED={VehicleStatus.ARMING_STATE_ARMED})"
            )
        ...
        # Simple horizontal avoidance: repulsion if within avoid_radius
        v_avoid = np.zeros(3, dtype=float)
        for person in self.people_ned:
            dp = p - person  # vector from person to vehicle (NED)
            dist_h = math.hypot(dp[0], dp[1])
            if dist_h < self.avoid_radius and dist_h > 1e-3:
                # push horizontally away from the person, proportional to penetration depth
                dir_h = np.array([dp[0] / dist_h, dp[1] / dist_h, 0.0])
                v_avoid += self.k_avoid * (self.avoid_radius - dist_h) * dir_h
        v_cmd += v_avoid
    ...
    # --- Vehicle commands ---
    def send_vehicle_command(self, command: int, param1=0.0, param2=0.0, param3=0.0, param4=0.0, param5=0.0, param6=0.0, param7=0.0) -> None:
        msg = VehicleCommand()
        msg.timestamp = int(self.get_clock().now().nanoseconds / 1000)
    ...
    def arm(self) -> None:
        # VEHICLE_CMD_COMPONENT_ARM_DISARM (400): param1=1 arm, 0 disarm
    ...
    def set_offboard_mode(self) -> None:
        # VEHICLE_CMD_DO_SET_MODE (176): param1=1 (custom), param2=6 (PX4_CUSTOM_MAIN_MODE_OFFBOARD)
