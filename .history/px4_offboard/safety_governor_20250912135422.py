...
class SafetyGovernor(Node):
    ...
    def __init__(self) -> None:
        ...
        # --- Rejoin-to-line behavior (after circle exit) ---
        self.rejoin_active = False
        self.rejoin_lookahead = 1.0  # meters ahead along base line when rejoining
        self.rejoin_tol = 0.15       # meters lateral tolerance to consider rejoined
    # default line-follow lookahead when not avoiding
    ...
    def cmdloop_callback(self) -> None:
        ...
        # If we have just exited the circle, guide the vehicle back to the base line first
        if (not self.circle_active) and self.rejoin_active:
            a_xy = np.array([self.wp_a[0], self.wp_a[1]], dtype=float)
            b_xy = np.array([self.wp_b[0], self.wp_b[1]], dtype=float)
            d = b_xy - a_xy
            ...
            if L < 1e-6:
                ...
                u_line = d / L
                # project current position onto the base segment
                s = float(np.dot(p_xy - a_xy, u_line))
                s = max(0.0, min(L, s))
                # choose lookahead direction based on current target
                dir_sign = 1.0 if np.allclose(self.current_target, self.wp_b) else -1.0
                s2 = max(0.0, min(L, s + dir_sign * self.rejoin_lookahead))
                rejoin_xy = a_xy + u_line * s2
                nav_target = np.array([rejoin_xy[0], rejoin_xy[1], self.current_target[2]], dtype=float)
                # clear rejoin once lateral error to base line is small
                foot_xy = a_xy + u_line * s
                lat_err = float(np.linalg.norm(p_xy - foot_xy))
                if lat_err <= self.rejoin_tol:
                    self.rejoin_active = False
