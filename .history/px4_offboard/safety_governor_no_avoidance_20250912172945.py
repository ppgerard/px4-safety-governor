...
class SafetyGovernorNoAvoidance(Node):
    ...
    # Declare parameters so they can be tuned at runtime
    self.declare_parameter('safety_radius', 4.0)   # m
    self.declare_parameter('v_cap_near', 3.0)      # m/s
    self.declare_parameter('v_cap_free', 8.0)      # m/s
    self.declare_parameter('debug_safety', False)  # bool

    self.safety_radius = float(self.get_parameter('safety_radius').value)
    self.v_cap_near = float(self.get_parameter('v_cap_near').value)
    self.v_cap_free = float(self.get_parameter('v_cap_free').value)
    self.debug_safety = bool(self.get_parameter('debug_safety').value)
